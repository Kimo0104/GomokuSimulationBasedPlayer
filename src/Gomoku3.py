...
def rules(self, color, pos):
    ...
    for move in self.board.get_empty_points:
        for two_directions in [["N","S"], ["NE","SW"], ["E","W"], ["SE","NW"]]:
            stats = self.line_rule(color, pos, two_directions)
            if stats["win"]:
                win.append(move)
            elif stats["block"]:
                block.append(move)
    ...
    random.shuffle(open_four)
    random.shuffle(block_open_four)
    random.shuffle(random)

    moves = win+block+open_four+block_open_four+random
    if len(moves) == 0:
        return "draw"
    return moves[0]
